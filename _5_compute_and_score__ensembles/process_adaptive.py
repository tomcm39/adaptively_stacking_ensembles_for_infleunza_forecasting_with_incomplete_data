...
def computeDEVIEnsembleWeights(trainingData,PRIOR):
    ...
    if nObs == 0:
        ...
        PRIOR = PRIOR/numModels
        alphas, elbos = deviMM(dataLogScores = np.array(modelsOnlyNoNA)
                               ,priorPis = PRIOR*np.array([1.]*numModels)
                               ,maxIters = 10**4
                               ,relDiffThreshold = -1
        )
        pis = alphas/sum(alphas)
        for (model,pi) in zip(modelNamesNoNA, pis):
            modelPis[model] = float(pi)
        modelPis['N'] = nObs
    return pd.Series(modelPis)

def computeDeEMEnsembleWeights(trainingData):
    modelsOnly = subsetColumns2Models(trainingData) 
    modelNames = modelsOnly.columns 
    ...
    nObs,numModels = modelsOnlyNoNA.shape
    if nObs == 0:
        modelPis = pd.Series({modelName:-999 for modelName in modelNames})
        modelPis['N'] = nObs
    else:
        alphas,Z,LLS = em(dataLogScores = np.array(modelsOnlyNoNA)
                          ,pis = np.array([1./numModels]*numModels)
                          ,maxIters = 10**4
                          ,relDiffThreshold = -1
        )
        pis = alphas/sum(alphas)
...
def computeWeightsFromTrainingData(trainingData, priorPercent, algorithm):
    ...
    weightDataForAppending['prior'] = priorPercent
...
if __name__ == "__main__":
    ...
    parser.add_argument("--prior"  , help="prior for adaptive model", nargs='?', type=int, default=0)
    ...
    for week in evalWeeks:
        ...
        if args.preSeason:
            if fromEW2Season(week) != '2010/2011': #only analyze this season
                continue
        else:
        ...
        priorPercent = 10**-5 if priorPercent == 0 else priorPercent
        PRIOR = priorPercent*N

        weightDataForForecasting, weightDataForAppending = computeWeightsFromTrainingData(trainingData, priorPercent=priorPercent, algorithm = lambda trainingData: computeDEVIEnsembleWeights(trainingData,PRIOR))
    ...
    if args.preSeason:
        allWtData.to_csv('./analysisData_preSeason/ensembleWeights__adaptweight_{:6.4f}.csv'.format(priorPercent))
        allLogScores.to_csv('./analysisData_preSeason/ensembleScores__adaptscore_{:6.4f}.csv'.format(priorPercent))
       
    else:
        allWtData.to_csv('./analysisData/ensembleWeights__adaptweight_{:6.4f}.csv'.format(priorPercent))
        allLogScores.to_csv('./analysisData/ensembleScores__adaptscore_{:6.4f}.csv'.format(priorPercent))
