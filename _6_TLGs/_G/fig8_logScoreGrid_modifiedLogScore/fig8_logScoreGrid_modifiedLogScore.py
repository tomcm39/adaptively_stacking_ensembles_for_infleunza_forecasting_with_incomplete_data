...
if __name__ == "__main__":
    ...
    overallAverage = readData('./plotData/averagedOverAll5.pq')
    
    FSNdynBySeas,FSNstaBySeas = readData("./plotData/dynamicBySeason5.pq"), readData("./plotData/staticBySeason5.pq")
    ...
    FSNEWBySeas = addSeason(FSNEWBySeas).groupby('season').apply(lambda x: pd.Series({'avgLogScore':np.mean(x.logScore5)})).reset_index()
    FSNEWBySeas['model'] = 'EW'
 
    FSNEWOverall = computeEWAvgLogScore(FSN=1)
    average = pd.Series({'avgLogScore':np.mean(FSNEWOverall.logScore5)})
    ...
    sns.heatmap(heatMapData
                ...
                ,linewidth=0.50)
    ax.set_ylabel('')
    ax.set_yticklabels(ax.get_yticklabels(),fontsize=7.5,va='center',ha='center')
    ...
    plt.savefig('fig8_logScoreGrid_modifiedLogScore.pdf')
    plt.savefig('fig8_logScoreGrid_modifiedLogScore.png')
    plt.savefig('fig8_logScoreGrid_modifiedLogScore.eps')
    plt.close()
