...
d <- read.csv('../scoreEnsembleModelsAgainstRealTimeRevisedData/analysisData/allDynamicAndStaticAndEqualScores.csv')

dynamic <- d[d$modelType=='dynamic',]
dynamic08 <- dynamic[dynamic$prior==0.08,]
dynamic08 <- rename(dynamic08,'logScore','logScoreDynamic')

static  <- d[d$modelType=='static',]
static00 <- static[static$prior==0.00,]
static00 <- rename(static00,'logScore','logScoreStatic')

equal   <- d[d$modelType=='equal',]
equal   <- rename(equal,'logScore','logScoreEqual') 

DS <- merge(dynamic08,static00,by = c('region','target','EWNum'))
DSE <- merge(DS,equal,by = c('region','target','EWNum'))
